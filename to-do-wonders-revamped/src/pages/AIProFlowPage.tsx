...
import { Button, InputField, Container, FileUpload, LoadingSpinner, ProBadge, Modal } from '../components/atoms';
...
export default function AIProFlowPage() {
  ...
  const updateRecruiterCount = (count: number) => {
  ...
  const handleResumeUpload = (files: File | File[]) => {
  ...
  const generateEmails = async () => {
    ...
    setTimeout(() => {
      const mockEmails: GeneratedEmail[] = formData.recruiters.map((recruiter, index) => ({
        id: index + 1,
        to: recruiter.email,
...
[Your Name]`,
  ...
  const viewEmail = (email: GeneratedEmail) => {
  ...
  const sendEmail = (emailId: number) => {
    if (!isOAuthConnected) {
